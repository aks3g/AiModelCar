...
static uint16_t sSensorDataToText(uint32_t timestamp, uint8_t duty, void *data, char *string, uint16_t len);
...
void updateUserland(void)
{
	...
	if (sGetFlagOnce(sFlagSensorLogIsFull)) {
		...
		for (int i=0 ; i<LOG_RECODE_SIZE ; ++i) {
			uint32_t len = sSensorDataToText(sSensorLog[idx][i].timeStamp, sSensorLog[idx][i].duty, &sSensorLog[idx][i].data, str, SENSOR_RECODE_TEXT_LEN+1);
			str += len;
		}
